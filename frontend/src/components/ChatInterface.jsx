...
const ChatInterface = ({ onBackToHome }) => {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      // Analyze image with language parameter
      const formData = new FormData()
    ...
    <div className="flex flex-col h-[calc(100vh-200px)] max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-t-3xl shadow-sm border-b border-gray-100 px-6 py-3 flex items-center space-x-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <span className="text-sm text-gray-600">{t('kisanBotOnline')}</span>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gradient-to-b from-primary-50/30 to-white">
        {messages.length === 0 && (
          <div className="text-center text-gray-400 mt-20">
            <h3 className="text-2xl font-bold mb-2">{t('startConversation')}</h3>
            <p>{t('conversationSubtitle')}</p>
          </div>
        )}
                ...
                <>
                  {msg.content.transcription && (
                    <p className="text-sm text-gray-600 mb-2 italic">
                      {t('youSaid')}: "{msg.content.transcription}"
                    </p>
                  )}
                      ...
                      {playingAudioId === msg.id ? (
                        <>
                          {isPaused ? (
                            ...
                            >
                              <FaPlay size={14} />
                              <span className="text-sm">{t('resume')}</span>
                            </button>
                          ) : (
                            <button
                              onClick={pauseAudio}
                        ...
                        >
                          <FaVolumeUp size={14} />
                          <span className="text-sm">{t('playAudio')}</span>
                        </button>
                      )}
                    </div>
                  )}
